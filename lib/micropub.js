...
module.exports.handleMessage = function(roomId, userId, body) {
    ...
    if (body.indexOf("!indieauth ") === 0) {
        ...
        db.getUser(userId).then(function(usr) {
            ...
            // console.log("Using token %s", user.oauth_token);
            ...
            request.post({
                ...
                if (!err && res.statusCode < 300) {
                    ...
                    client.sendMessage(roomId, {
                        body: "Success!" + (postLocation ? (" Located at: " + postLocation) : ""),
                        msgtype: "m.notice"
                    });
                }
                else {
...
var doIndieAuthVerify = function(state, code, me) {
    ...
    db.getSession(state).done(function(session) {
        // get an access token for the code
        request.post({
            ...
            for (var i = 0; i < vals.length; i++) {
                if (vals[i].indexOf("access_token=") === 0) {
                    token = vals[i].substring("access_token=".length);
                }
            }

            db.storeUser(session.user_id, session.me, token).done(function() {
                ...
                db.getAdminRoom(session.user_id).done(function(room) {
                    client.sendMessage(room.room_id, {
                        body: "Authorised as " + session.me,
                        msgtype: "m.notice"
                    });
                }, function(e) {
                    console.error(e);
